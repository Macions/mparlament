...
export default function EditVoting() {
		...
		async function fetchUser() {
			try {
				...
				if (response.ok) {
					...
					setIsAdmin(
						user.role === "admin" ||
							user.permissions?.includes("MANAGE_VOTINGS"),
					);
				}
			} catch (err) {
	...
	useEffect(() => {
		async function fetchData() {
			try {
				...
				const membersRes = await fetch("/newapp/api/users", {
					headers: { Authorization: `Bearer ${token}` },
				});
				if (membersRes.ok) {
				...
				if (resolutionsRes.ok) {
					...
					if (rawData && typeof rawData === "object") {
						if (Array.isArray(rawData)) {
							...
							rawData.resolutions &&
							Array.isArray(rawData.resolutions)
						) {
				...
				if (
					...
					const amendment = amendmentsData.find(
		...
		if (step === 2) {
			...
			if (
				(formData.recipientsType === "individual" ||
					formData.recipientsType === "members") &&
	...
	const handleSubmit = async () => {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					data.message || "Nie udało się zaktualizować głosowania",
				);
			}
			...
			if (newFiles.length > 0) {
				...
				await fetch(`/newapp/api/votings/${id}/attachments`, {
					method: "POST",
					headers: {
						Authorization: `Bearer ${token}`,
					},
						...
						<div className="form-group">
							...
							>
								<p style={{ margin: 0 }}>
									...
									samych praw - nikt nie może Ci ich odebrać. Administratorzy
									nie są wyświetlani na liście, ponieważ mają pełne uprawnienia
									do wszystkich głosowań.
								</p>
							</div>
