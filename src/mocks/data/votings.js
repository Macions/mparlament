export let votings = [
	{
		id: 1,
		title: "Uchwała w sprawie zwiększenia finansowania oświaty",
		description:
			"Projekt uchwały dotyczący zwiększenia środków przeznaczonych na edukację.",
		category: "resolution",
		startTime: "2026-07-11T16:19:00",
		endTime: "2026-07-11T22:00:00",
		status: "active",
		recipientsType: "all",
		selectedGroups: [],
		selectedMembers: [],
		linkedItemType: "none",
		linkedItemId: "",
		applicant: "marshal",
		attachments: [],
		createdBy: "Jan Kowalski",
		managers: [3],
		assignedTo: "all",
		isAnonymous: false,
	},
	{
		id: 2,
		title: "Zmiana w ustawie o ochronie środowiska",
		description: "Zmiana przepisów dotyczących emisji.",
		category: "law",
		startTime: "2026-07-06T16:00:00",
		endTime: "2026-07-06T18:00:00",
		status: "finished",
		recipientsType: "groups",
		selectedGroups: [2, 4],
		selectedMembers: [],
		linkedItemType: "resolution",
		linkedItemId: 2,
		applicant: "presidium",
		attachments: [
			{
				id: 1,
				name: "projekt_zmian.pdf",
				size: 1024 * 1024 * 2.5,
				type: "application/pdf",
				uploadDate: "2026-07-05T10:00:00",
			},
		],
		createdBy: "Marszałek Parlamentu",
		managers: [],
		assignedTo: "groups",
		isAnonymous: true,
	},
	{
		id: 3,
		title: "Uchwała w sprawie budżetu na 2026 rok",
		description: "Głosowanie nad budżetem na rok 2026.",
		category: "budget",
		startTime: "2026-06-20T10:00:00",
		endTime: "2026-06-20T14:00:00",
		status: "archived",
		recipientsType: "groups",
		selectedGroups: [1, 3, 5],
		selectedMembers: [],
		linkedItemType: "none",
		linkedItemId: "",
		applicant: "group_15",
		attachments: [],
		createdBy: "Anna Nowak",
		managers: [],
		assignedTo: "groups",
		isAnonymous: false,
	},
	{
		id: 4,
		title: "Zmiana regulaminu Parlamentu Młodych RP",
		description:
			"Proponowane zmiany w regulaminie dotyczące procedury głosowania.",
		category: "amendment",
		startTime: "2026-07-12T09:00:00",
		endTime: "2026-07-12T12:00:00",
		status: "upcoming",
		recipientsType: "members",
		selectedGroups: [],
		selectedMembers: [3],
		linkedItemType: "amendment",
		linkedItemId: 1,
		applicant: "",
		attachments: [
			{
				id: 2,
				name: "regulamin_nowy.docx",
				size: 1024 * 1024 * 1.2,
				type: "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
				uploadDate: "2026-07-11T08:00:00",
			},
		],
		createdBy: "Przewodniczący Komisji",
		managers: [],
		assignedTo: "members",
		isAnonymous: false,
	},
	{
		id: 5,
		title: "Uchwała dotycząca modernizacji infrastruktury",
		description:
			"Projekt uchwały w sprawie modernizacji infrastruktury technicznej.",
		category: "other",
		startTime: "2026-07-08T14:00:00",
		endTime: "2026-07-08T16:30:00",
		status: "finished",
		recipientsType: "all",
		selectedGroups: [],
		selectedMembers: [],
		linkedItemType: "none",
		linkedItemId: "",
		applicant: "marshal",
		attachments: [],
		createdBy: "Minister Infrastruktury",
		managers: [],
		assignedTo: "all",
		isAnonymous: false,
	},
	{
		id: 6,
		title: "Program wsparcia dla młodych przedsiębiorców",
		description:
			"Głosowanie nad programem wsparcia finansowego dla młodych przedsiębiorców.",
		category: "other",
		startTime: "2026-07-05T10:00:00",
		endTime: "2026-07-05T14:00:00",
		status: "archived",
		recipientsType: "members",
		selectedGroups: [],
		selectedMembers: [2, 4, 6, 8, 10, 12],
		linkedItemType: "resolution",
		linkedItemId: 3,
		applicant: "presidium",
		attachments: [
			{
				id: 3,
				name: "program_wsparcia.pdf",
				size: 1024 * 1024 * 3.0,
				type: "application/pdf",
				uploadDate: "2026-07-04T09:00:00",
			},
			{
				id: 4,
				name: "załącznik_1.xlsx",
				size: 1024 * 1024 * 0.8,
				type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
				uploadDate: "2026-07-04T09:30:00",
			},
		],
		createdBy: "Marszałek Parlamentu",
		managers: [],
		assignedTo: "members",
		isAnonymous: true,
	},
	{
		id: 7,
		title: "Głosowanie nad poprawką do uchwały o finansowaniu oświaty",
		description:
			"Głosowanie nad poprawką nr 1 do uchwały w sprawie zwiększenia finansowania oświaty, dotyczącą zwiększenia budżetu na szkolenia z kompetencji cyfrowych.",
		category: "amendment",
		startTime: "2026-07-16T10:00:00",
		endTime: "2026-07-16T23:16:00",
		status: "active",
		recipientsType: "all",
		selectedGroups: [],
		selectedMembers: [],
		linkedItemType: "amendment",
		linkedItemId: 1,
		applicant: "marshal",
		attachments: [
			{
				id: 5,
				name: "poprawka_nr1.pdf",
				size: 1024 * 1024 * 0.5,
				type: "application/pdf",
				uploadDate: "2026-07-14T10:00:00",
			},
		],
		createdBy: "Anna Nowak",
		managers: [],
		assignedTo: "all",
		isAnonymous: false,
	},
	{
		id: 8,
		title: "Pakiet poprawek do uchwały o finansowaniu oświaty",
		description:
			"Głosowanie zbiorcze nad poprawkami zgłoszonymi do uchwały w sprawie zwiększenia finansowania oświaty.",
		category: "amendment",
		votingMode: "batch",
		questions: [
			{
				id: "q_8_1",
				text: "Czy jesteś za przyjęciem poprawki nr 1 (zwiększenie budżetu na szkolenia z kompetencji cyfrowych)?",
				linkedItemType: "amendment",
				linkedItemId: 1,
				resolutionId: 1,
			},
			{
				id: "q_8_2",
				text: "Czy jesteś za przyjęciem poprawki nr 5 (zmiana nazwy rozdziału 1)?",
				linkedItemType: "amendment",
				linkedItemId: 5,
				resolutionId: 1,
			},
			{
				id: "q_8_3",
				text: "Czy jesteś za przyjęciem uchwały jako całości?",
				linkedItemType: "resolution",
				linkedItemId: 1,
				resolutionId: "",
			},
		],
		startTime: "2026-09-18T19:00:00",
		endTime: "2026-09-25T19:00:00",
		status: "active",
		recipientsType: "all",
		selectedGroups: [],
		selectedMembers: [],
		linkedItemType: "none",
		linkedItemId: "",
		applicant: "marshal",
		attachments: [],
		createdBy: "Jan Kowalski",
		managers: [3],
		assignedTo: "all",
		isAnonymous: false,
	},
];
